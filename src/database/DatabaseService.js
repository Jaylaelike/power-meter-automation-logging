...
class DatabaseService {
     ...
     * @param {string} stationData.uuid - Station UUID
    ...
    async createStation(stationData) {
        return this.executeOperation(async (prisma) => {
            const { name, uuid, ipAddress, scene } = stationData;
            
            // Validate required fields
            if (!name || !uuid || !ipAddress || !scene) {
                throw new Error('Missing required station fields: name, uuid, ipAddress, scene');
            }
    ...
    async deleteStation(stationId) {
        return this.executeOperation(async (prisma) => {
            ...
            if (readingCount > 0) {
                throw new Error(`Cannot delete station: ${readingCount} power readings exist. Delete readings first.`);
            }
    ...
    async findOrCreateStation(stationConfig) {
        return this.executeOperation(async (prisma) => {
            const { name, uuid, ip, scene } = stationConfig;
            ...
            if (station) {
                // Update station if configuration has changed
                const needsUpdate = 
                    station.uuid !== uuid.trim() ||
                    station.ipAddress !== ip.trim() ||
                    station.scene !== scene.trim();
    ...
    async createBatchPowerReadings(readingsData) {
        return this.executeOperation(async (prisma) => {
            ...
            const dbReadings = readingsData.map(reading => {
                ...
                // Validate required fields
                if (!stationId || !timestamp) {
                    throw new Error('Missing required fields in batch data: stationId, timestamp');
                }
    ...
    transformPowerData(powerData) {
        ...
        // Map MUX Power Meter readings (IDs 18069, 18070, 73909, 73910)
        const muxPowerMap = {
            18069: 'muxPower1', // TV5
        ...
        Object.entries(powerData).forEach(([id, value]) => {
            ...
            if (isNaN(numValue)) {
                ...
                if (idNum >= 8684 && idNum <= 8689) {
                    // Active Power should be positive and reasonable (0-50000 W)
                    if (numValue < 0 || numValue > 50000) {
                        result.warnings.push(`Active Power ID ${id} value seems out of range: ${numValue}W`);
                    }
                } else if ([18069, 18070, 73909, 73910].includes(idNum)) {
                    // MUX Power should be positive and reasonable (0-10000000 kWh)
                    if (numValue < 0 || numValue > 10000000) {
                        result.warnings.push(`MUX Power ID ${id} value seems out of range: ${numValue}kWh`);
                    }
                }
            }
        });
     ...
     */
    async getPowerReadingsByTimeRange(stationId, startDate, endDate, options = {}) {
        return this.executeOperation(async (prisma) => {
            const { limit = 1000, offset = 0, orderBy = 'timestamp', orderDirection = 'desc' } = options;
    ...
    async getPowerReadingStats(stationId, startDate = null, endDate = null) {
        return this.executeOperation(async (prisma) => {
            ...
            return {
                ...
                timeSpan: firstReading && lastReading ? 
                    lastReading.timestamp.getTime() - firstReading.timestamp.getTime() : 0
            };
